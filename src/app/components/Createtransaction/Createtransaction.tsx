...
export default function Createtransaction({ onTransactionCreated }: CreatetransactionProps): JSX.Element {
  ...
  useEffect(() => {
    const fetchAccounts = async () => {
      if (!token) { setAccountsLoading(false); setAccountsError("Ekki innskráður."); return; }
      ...
      try {
        ...
        if (error instanceof ApiError && error.status === 401) { logout(); setAccountsError("Útskráð/ur."); }
        else if (error instanceof Error) { setAccountsError(`Villa: ${error.message}`); }
        else { setAccountsError("Óþekkt villa við að sækja reikninga."); }
  ...
  const clearForm = () => {
     ...
     setAmount(''); setDescription(''); setMessage(''); setErrorDetails(null);
     setUiState('initial');
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token || !user) { setMessage('Notandi ekki innskráður.'); setUiState('error'); return; }
    if (!account) { setMessage('Vinsamlegast veldu reikning.'); setUiState('error'); return; }

    setUiState('loading'); setMessage(''); setErrorDetails(null);
    ...
    if (isNaN(newTransactionData.account_id) || isNaN(newTransactionData.payment_method_id) || isNaN(newTransactionData.amount) || newTransactionData.amount <= 0) {
        setMessage('Ógild gögn. Athugaðu reikning, greiðslumáta og upphæð (> 0).');
        setUiState('error'); return;
    }
    if (!newTransactionData.transaction_type || !newTransactionData.category || !newTransactionData.description) {
        setMessage('Vinsamlegast fylltu út alla reiti.'); setUiState('error'); return;
    }

    const apiClient = new ApiClient(); apiClient.setToken(token);

    try {
      const createdTransaction = await apiClient.createTransaction(newTransactionData);
      setUiState('data'); setMessage('Færsla stofnuð!');
      console.log('Transaction created:', createdTransaction);
      onTransactionCreated();
      setTimeout(clearForm, 2500);
    } catch (error: any) {
      console.error('Error creating transaction:', error); setUiState('error');
      if (error instanceof ApiError) {
           setMessage(`Villa (${error.status}): ${error.message}`);
           if (error.details?.details) { 
               setErrorDetails(error.details.details);
               setMessage(`Villa (${error.status}): Ógild færslugögn. Sjá nánar hér að neðan.`);
           }
           if (error.status === 401) { logout(); }
      } else if (error instanceof Error) { setMessage(`Villa: ${error.message}`); }
      else { setMessage('Óþekkt villa við að stofna færslu.'); }
    }
  };

  if (uiState === 'loading') { return <p className={styles.loading}>Stofna færslu...</p>; }

  return (
    <div className={styles.formContainer}>
      <h2 className={styles.title}>Stofna nýja færslu</h2>
      {accountsError && <p className={styles.errorMessage}>{accountsError}</p>}
      {message && !errorDetails && (<p className={uiState === 'error' ? styles.errorMessage : styles.successMessage}>{message}</p>)}
      {errorDetails && (
         <div className={styles.errorDetails}>
            <p className={styles.errorMessage}>{message.includes("Sjá nánar") ? message : "Gögn eru ógild:"}</p>
            <ul>
               {Object.entries(errorDetails.fieldErrors || {}).map(([field, errors]) => (<li key={field}><strong>{field}:</strong> {(errors as string[]).join(', ')}</li>))}
               {(errorDetails.formErrors || []).map((err: string, index: number) => (<li key={`form-${index}`}>{err}</li>))}
            </ul>
         </div>
       )}
       ...
       <form className={styles.form} onSubmit={handleSubmit}>
         ...
         <div className={styles.buttonGroup}>
            <button type="submit" className={styles.submitButton} disabled={uiState === 'loading' || accountsLoading}>
              {uiState === 'loading' ? 'Sendi...' : 'Stofna færslu'}
            </button>
             <button type="button" onClick={clearForm} className={styles.clearButton} disabled={uiState === 'loading'}>
                 Hreinsa
             </button>
         </div>
       </form>
